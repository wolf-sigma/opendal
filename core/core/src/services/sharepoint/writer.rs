...
impl SharePointWriter {
    ...
    pub fn new(core: Arc<SharePointCore>, op: OpWrite, path: String) -> Self {
        SharePointWriter { core, op, path }
    }
}
...
impl SharePointWriter {
    async fn write_simple(&self, bs: Buffer) -> Result<Metadata> {
        let response = self
            .core
            .sharepoint_upload_simple(&self.path, &self.op, bs)
            .await?;
    ...
    async fn create_upload_session(&self) -> Result<OneDriveUploadSessionCreationResponseBody> {
        let response = self
            .core
            .sharepoint_create_upload_session(&self.path, &self.op)
            .await?;
        match response.status() {
            StatusCode::OK => {
